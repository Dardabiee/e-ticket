 ...
 @section('title','Landing Page')
  ...
  <section id="heroSection" class="container relative max-w-screen-xl pt-10 pb-24">
    <div class="flex flex-wrap items-center justify-between w-full gap-1">
        ...
        <img src="{{ asset('assets/images/landingpek.png') }}" class="max-w-[600px] max-h-[400px] w-[70%] h-[70%]"
            alt="tickety-assets">
    </div>
</section>
  <!-- Wavy line ornament -->
  ...
  <section id="eventSection" class="container relative max-w-screen-xl py-10">
    ...
    <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-[30px]">
        ...
        <x-frontend.card-event :cover="$event->thumbnail" :title="$event->name" :isPopuler="$event->is_populer" 
        :category="$event->category->name" :date="$event->start_time" :price="$event->start_from" :description="$event->description" :route="route('detail', $event->slug)" />
  ...
  <section id="categoriesSection" class="relative pb-[100px] overflow-hidden">
    <div class="container relative max-w-screen-xl py-10">
        ...
        <div class="grid sm:grid-cols-2 lg:grid-cols-4 gap-[30px] relative">
          ...
          :totalEvent="$category->events_count" :route="request()->fullUrlWithQuery(['category' => $category->id])"/>
          @endforeach
